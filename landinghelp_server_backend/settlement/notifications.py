...
def _can_send_contract_package(user):
    ...
    allowed_roles = {
        ...
        getattr(role_cls, 'HQ_STAFF', 'HQ_STAFF'),
    }
...
def _get_or_create_shared_conversation(submission, subject_fallback=''):
    ...
    """
    from messaging.workspace import get_or_create_hq_workspace
    conv = get_or_create_hq_workspace(submission)
    if subject_fallback and not conv.subject:
        conv.subject = subject_fallback
        conv.save(update_fields=['subject'])
    return conv


def _get_or_create_workspace_conversation(submission, workspace_type, subject_fallback='', agent=None):
    ...
    if workspace_type == Conversation.WorkspaceType.LOCAL_EXECUTION:
        conv = get_or_create_local_workspace(submission, agent=agent)
    elif workspace_type == Conversation.WorkspaceType.HQ_BACKOFFICE:
        conv = get_or_create_hq_workspace(submission)
    else:
        conv = _get_or_create_shared_conversation(submission, subject_fallback=subject_fallback)
...
def send_contract_package_to_customer(submission, documents, sender=None, language_code='ko'):
    """
    HQ_BACKOFFICE 워크스페이스로 계약 패키지 안내 메시지 발송.
    sender가 지정되면 SUPER_ADMIN/ADMIN/SUPERVISOR/HQ_STAFF만 허용.
    """
    if not submission:
        return False

    if sender is not None and not _can_send_contract_package(sender):
        logger.warning(
            "Contract package send denied: submission_id=%s sender_id=%s",
    ...
    try:
        ...
        conv = _get_or_create_workspace_conversation(
            submission,
            Conversation.WorkspaceType.HQ_BACKOFFICE,
            subject_fallback=_get_display_text('계약 패키지 안내', language_code),
        )

        message_sender = sender or _get_system_sender()
        if not message_sender:
            return False

        lines = [
            _get_display_text('견적 수락 후 계약 패키지가 생성되었습니다.', language_code),
            _get_display_text('아래 문서를 확인하고 서명 절차를 진행해 주세요.', language_code),
            '',
        ]
        for document in (documents or []):
            doc_label = getattr(document, 'get_document_type_display', lambda: '')() or getattr(document, 'document_type', 'DOCUMENT')
            lines.append(f"- {doc_label} (#{getattr(document, 'id', '-')})")

        body = '\n'.join(lines)
        Message.objects.create(conversation=conv, sender=message_sender, body=body)
        return True
    except Exception as e:
        logger.warning(
            "Contract package send failed: submission_id=%s error=%s",
            getattr(submission, 'id', None),
            e,
            exc_info=True,
        )
...
def send_signed_final_package_delivery(submission, final_package_document, sender=None, language_code='ko'):
    """최종 서명 패키지 고객 전달 메시지(HQ 워크스페이스)."""
    if not submission or not final_package_document:
        return False
    ...
    try:
        ...
        conv = _get_or_create_workspace_conversation(
            submission,
            Conversation.WorkspaceType.HQ_BACKOFFICE,
            subject_fallback=_get_display_text('최종 서명 패키지 안내', language_code),
        )

        message_sender = sender if _can_send_contract_package(sender) else _get_system_sender()
        if not message_sender:
            return False

        lines = [
            _get_display_text('필수 서명이 모두 완료되었습니다.', language_code),
            _get_display_text('최종 서명 패키지가 생성되어 보관 및 전달되었습니다.', language_code),
            f"- {_get_display_text('문서 유형', language_code)}: {final_package_document.get_document_type_display()}",
            f"- {_get_display_text('문서 ID', language_code)}: #{getattr(final_package_document, 'id', '-')}",
        ]
        Message.objects.create(conversation=conv, sender=message_sender, body='\n'.join(lines))
        return True
    except Exception as e:
        logger.warning(
            "Signed final package delivery send failed: submission_id=%s document_id=%s error=%s",
            getattr(submission, 'id', None),
            getattr(final_package_document, 'id', None),
            e,
            exc_info=True,
        )
        return False
