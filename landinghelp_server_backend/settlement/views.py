...
def customer_quote(request):
    ...
    if quote.submission_id:
        ...
        contract_package_documents = [_serialize_contract_package_document(d, request) for d in docs]
        signed_final_doc = (
            CaseDocument.objects.filter(
                submission_id=quote.submission_id,
        ...
        if signed_final_doc:
            signed_final_package_document = _serialize_contract_package_document(signed_final_doc, request)
        service_completion_forms = [
            _serialize_service_completion_form(row, request)
            for row in _service_completion_forms_visible_to_user(request.user, submission_id=quote.submission_id)
        ]
    ...
    return render(request, 'services/customer_quote.html', {
        ...
        'service_completion_forms': service_completion_forms,
...
def api_my_quote(request):
    ...
    signed_final_package_document = None
    service_completion_forms = []
    if quote and quote.submission_id:
        docs = (
            CaseDocument.objects.filter(
                submission_id=quote.submission_id,
                document_type__in=[
...
@require_POST
@ensure_csrf_cookie
def api_quote_accept(request):
    """고객 견적 수락(결제와 분리). QUOTE_ACCEPTED 단계로 전이하고 계약 패키지 문서를 생성한다."""
    if not request.user.is_authenticated:
    ...
    if getattr(request.user, 'role', None) == 'AGENT':
        ...
        return JsonResponse({'ok': False, 'error': '고객만 견적 수락을 할 수 있습니다.'}, status=403)
    ...
    try:
        from .quote_acceptance import accept_quote
        result = accept_quote(request.user, quote_id=quote_id)
    except Exception as exc:
        if _is_form_post(request):
            return redirect(reverse('customer_quote') + '?error=invalid')
        return JsonResponse({'ok': False, 'error': str(exc)}, status=400)

    redirect_url = reverse('customer_quote')
    if _is_form_post(request):
        return redirect(redirect_url)
    return JsonResponse({
        'ok': True,
        'message': '견적을 수락했습니다. 계약 패키지를 확인해 주세요.',
        'submission_id': getattr(result.get('submission'), 'id', None),
...
def _serialize_contract_package_document(doc, request):
    file_url = ''
    try:
        if doc.file:
    ...
    required_signer_roles = [str(r).strip().upper() for r in (metadata.get('required_signer_roles') or []) if str(r).strip()]
    signed_roles = [str(r).strip().upper() for r in (metadata.get('signed_roles') or []) if str(r).strip()]

    user = getattr(request, 'user', None)
    is_customer_owner = bool(
        user
        and getattr(user, 'is_authenticated', False)
        and getattr(user, 'role', None) == getattr(getattr(user, 'Role', None), 'CUSTOMER', 'CUSTOMER')
        and getattr(doc.submission, 'user_id', None) == getattr(user, 'id', None)
    )
    is_internal = bool(user and getattr(user, 'is_authenticated', False) and getattr(user, 'is_internal_staff', lambda: False)())

    can_sign_as_customer = is_customer_owner and 'CUSTOMER' in required_signer_roles and 'CUSTOMER' not in signed_roles
    can_sign_as_internal = is_internal and 'INTERNAL' in required_signer_roles and 'INTERNAL' not in signed_roles
    ...
    return {
        ...
        'customer_action_required': bool(metadata.get('customer_action_required')),
        'required_signer_roles': required_signer_roles,
        'signed_roles': signed_roles,
        'can_sign_as_customer': can_sign_as_customer,
        'can_sign_as_internal': can_sign_as_internal,
        'signature_count': DocumentSignature.objects.filter(document=doc).count(),
        'created_at': doc.created_at.isoformat() if doc.created_at else '',
    }


def _service_completion_forms_visible_to_user(user, submission_id=None):
    if not user or not getattr(user, 'is_authenticated', False):
        return ServiceCompletionForm.objects.none()

    qs = ServiceCompletionForm.objects.select_related('submission', 'agent', 'schedule_plan', 'reviewed_by').order_by('-submitted_at', '-id')
    if submission_id:
        qs = qs.filter(submission_id=submission_id)

    if getattr(user, 'is_internal_staff', lambda: False)():
        return qs

    role = getattr(user, 'role', None)
    role_cls = getattr(user, 'Role', None)
    if role == getattr(role_cls, 'AGENT', 'AGENT'):
        return qs.filter(agent=user)
    if role == getattr(role_cls, 'CUSTOMER', 'CUSTOMER'):
        return qs.filter(submission__user=user, status=ServiceCompletionForm.Status.REVIEWED)
    return ServiceCompletionForm.objects.none()


def _serialize_service_completion_form(row, request):
    return {
        'id': row.id,
        'submission_id': row.submission_id,
        'agent_id': row.agent_id,
        'agent_name': (row.agent.get_full_name() or row.agent.username or row.agent.email or '') if row.agent_id else '',
        ...
        'reviewed_by_name': (
            (row.reviewed_by.get_full_name() or row.reviewed_by.username or row.reviewed_by.email or '')
            if row.reviewed_by_id else ''
        ),
    }


def _notify_service_completion_workspace(form, actor, event_key='submitted'):
    try:
        ...
        conv = _get_or_create_workspace_conversation(
            ...
            subject_fallback='서비스 완료 보고',
            agent=form.agent,
        )
        sender = actor if actor and getattr(actor, 'is_authenticated', False) else _get_system_sender()
        if not sender:
            return False

        if event_key == 'reviewed':
            body = '\n'.join([
                '서비스 완료 보고서가 내부 검토 완료 처리되었습니다.',
                f'- Agent: {(form.agent.get_full_name() or form.agent.username or form.agent.email or "-")}',
                f'- Summary: {form.summary or "-"}',
                f'- Status: {form.get_status_display()}',
            ])
        else:
            body = '\n'.join([
                'Agent 서비스 완료 보고서가 제출되었습니다.',
                f'- Agent: {(form.agent.get_full_name() or form.agent.username or form.agent.email or "-")}',
                f'- Summary: {form.summary or "-"}',
                f'- Attachments: {form.attachments_count}',
            ])
...
@require_GET
@ensure_csrf_cookie
def api_service_completion_forms(request):
    if not request.user.is_authenticated:
        return JsonResponse({'ok': False, 'error': '로그인이 필요합니다.'}, status=403)
    ...
    forms = _service_completion_forms_visible_to_user(request.user, submission_id=submission_id_int)
    payload = [_serialize_service_completion_form(row, request) for row in forms[:200]]
    return JsonResponse({'ok': True, 'forms': payload})


@require_POST
@ensure_csrf_cookie
def api_service_completion_submit(request):
    if not request.user.is_authenticated:
        return JsonResponse({'ok': False, 'error': '로그인이 필요합니다.'}, status=403)
    ...
    if role != getattr(role_cls, 'AGENT', 'AGENT'):
        return JsonResponse({'ok': False, 'error': 'Agent만 제출할 수 있습니다.'}, status=403)
    ...
    try:
        submission_id = int(body.get('submission_id'))
    except (TypeError, ValueError):
    ...
    if not submission:
        return JsonResponse({'ok': False, 'error': 'submission을 찾을 수 없습니다.'}, status=404)

    schedule_plan = None
    raw_plan_id = body.get('schedule_plan_id')
    if raw_plan_id is not None and str(raw_plan_id).strip() != '':
        try:
            schedule_plan_id = int(raw_plan_id)
        except (TypeError, ValueError):
            return JsonResponse({'ok': False, 'error': '유효한 schedule_plan_id가 필요합니다.'}, status=400)
        schedule_plan = ServiceSchedulePlan.objects.filter(id=schedule_plan_id, submission=submission).first()
        if not schedule_plan:
            return JsonResponse({'ok': False, 'error': '해당 submission의 schedule plan이 아닙니다.'}, status=400)

    eligible, reason, target_plan = ServiceCompletionForm.check_completion_eligibility(
    ...
    raw_attachments_count = body.get('attachments_count')
    try:
        attachments_count = max(0, int(raw_attachments_count or 0))
    except (TypeError, ValueError):
        return JsonResponse({'ok': False, 'error': 'attachments_count는 0 이상 정수여야 합니다.'}, status=400)

    form, _created = ServiceCompletionForm.objects.get_or_create(
        submission=submission,
        agent=request.user,
        schedule_plan=target_plan,
        defaults={
            'status': ServiceCompletionForm.Status.SUBMITTED,
            'submitted_at': timezone.now(),
        },
    )
    form.status = ServiceCompletionForm.Status.SUBMITTED
    form.summary = summary
    form.completion_notes = completion_notes
    form.attachments_count = attachments_count
    form.submitted_at = timezone.now()
    form.reviewed_at = None
    form.reviewed_by = None
    form.save(update_fields=[
        'status', 'summary', 'completion_notes', 'attachments_count',
        'submitted_at', 'reviewed_at', 'reviewed_by', 'updated_at',
    ])

    _notify_service_completion_workspace(form, request.user, event_key='submitted')
    return JsonResponse({
        'ok': True,
        'form': _serialize_service_completion_form(form, request),
    })


@require_POST
@ensure_csrf_cookie
def api_service_completion_review(request, form_id):
    if not request.user.is_authenticated:
        return JsonResponse({'ok': False, 'error': '로그인이 필요합니다.'}, status=403)
    if not getattr(request.user, 'is_internal_staff', lambda: False)():
        return JsonResponse({'ok': False, 'error': '내부 담당자만 검토할 수 있습니다.'}, status=403)

    form = (
        ServiceCompletionForm.objects
        .select_related('submission', 'agent', 'schedule_plan', 'reviewed_by')
        .filter(id=form_id)
        .first()
    )
    if not form:
        return JsonResponse({'ok': False, 'error': '완료 보고서를 찾을 수 없습니다.'}, status=404)
    ...
    _notify_service_completion_workspace(form, request.user, event_key='reviewed')
...
@require_POST
@ensure_csrf_cookie
def api_sign_case_document(request, document_id):
    if not request.user.is_authenticated:
        if _is_form_post(request):
            return redirect(reverse('customer_quote') + '?error=login')
        return JsonResponse({'ok': False, 'error': '로그인이 필요합니다.'}, status=403)
    ...
    document = (
        CaseDocument.objects.select_related('submission', 'submission__user')
    ...
    signer_role = (request.POST.get('signer_role') or '').strip().upper() if _is_form_post(request) else ''
    ip_address = request.META.get('HTTP_X_FORWARDED_FOR', '').split(',')[0].strip() or request.META.get('REMOTE_ADDR')
    user_agent = request.META.get('HTTP_USER_AGENT', '')

    try:
        from .document_signatures import sign_case_document

        result = sign_case_document(
            document=document,
            signer=request.user,
            signer_role=signer_role or None,
            audit_payload={
                'source': 'api_sign_case_document',
                'path': request.path,
        ...
        if _is_form_post(request):
            target = 'app_submission_review' if getattr(request.user, 'is_internal_staff', lambda: False)() else 'customer_quote'
            if target == 'app_submission_review':
                return redirect(reverse(target, kwargs={'submission_id': document.submission_id}) + '?error=sign')
            return redirect(reverse(target) + '?error=sign')
        return JsonResponse({'ok': False, 'error': str(exc)}, status=400)

    if _is_form_post(request):
        if getattr(request.user, 'is_internal_staff', lambda: False)():
            return redirect(reverse('app_submission_review', kwargs={'submission_id': document.submission_id}))
        return redirect(reverse('customer_quote'))

    final_package_document = result.get('final_package_document')
    return JsonResponse({
        'ok': True,
        'already_signed': bool(result.get('already_signed')),
        'required_complete': bool(result.get('required_complete')),
        'document': _serialize_contract_package_document(document, request),
        'final_package_document': _serialize_contract_package_document(final_package_document, request) if final_package_document else None,
    })
...
def api_complete_human_review(request, review_id):
    ...
    if not request.user.is_authenticated:
        return JsonResponse({'ok': False, 'error': '로그인이 필요합니다.'}, status=403)
    if not request.user.can_finalize_schedule():
        return JsonResponse({'ok': False, 'error': '권한이 없습니다.'}, status=403)
    import json
    note = ''
    customer_message = ''
...
def _customer_quote_i18n(lang):
    ...
    return {
        ...
        'accept_quote': get_display_text('견적 수락', lang),
        'contract_package': get_display_text('계약 패키지', lang),
        'contract_package_desc': get_display_text('견적 수락 후 생성된 계약 문서를 한 곳에서 확인할 수 있습니다.', lang),
        'signed_final_package': get_display_text('최종 서명 패키지', lang),
        'signed_final_package_desc': get_display_text('모든 필수 서명이 완료되면 최종 패키지가 생성되어 전달됩니다.', lang),
        'sign_document': get_display_text('서명하기', lang),
    }
