...
def _notify_agent_lsa(batch, agent, payload, lsa_request, language_code='ko'):
    ...
    from .notifications import _get_or_create_shared_conversation, _get_system_sender, _is_email_configured
    from django.conf import settings
    ...
    conv = _get_or_create_shared_conversation(batch.submission, subject_fallback='LSA 일정 제안 요청')
...
def _notify_agent_lsa_not_selected(batch, agent):
    ...
    conv = _get_or_create_shared_conversation(batch.submission, subject_fallback='LSA 결과 안내')
    ConversationParticipant.objects.get_or_create(conversation=conv, user=agent)
...
def _expand_existing_shared_conversation_for_contract(contract):
    ...
    """
    from messaging.models import ConversationParticipant, Message
    from .notifications import _get_or_create_shared_conversation, _get_system_sender
    ...
    conv = _get_or_create_shared_conversation(submission, subject_fallback='정착 서비스')
    _, created = ConversationParticipant.objects.get_or_create(conversation=conv, user=selected_agent)
    ...
    if created:
        ...
        Message.objects.create(
            conversation=conv,
            sender=sender,
            body=f"System: {display_name} joined the thread.",
        )
    return True
...
def _deliver_contract_to_selected_agent(contract):
    ...
    conv = _get_or_create_shared_conversation(contract.submission, subject_fallback='계약 확정 안내')
