...
class SurveySubmissionStateMachineTests(TestCase):
    ...
    def test_case_stage_helper_methods(self):
        sub = SurveySubmission.objects.create(
            user=self.user,
            email=self.user.email,
            status=SurveySubmission.Status.SUBMITTED,
        ...
        self.assertTrue(sub.can_customer_accept_quote())
        self.assertTrue(sub.is_ready_for_payment())
        self.assertFalse(sub.can_send_contract_package())
        self.assertFalse(sub.is_ready_for_availability_request())
        self.assertFalse(sub.is_ready_for_schedule_finalize())

        sub.advance_case_stage(SurveySubmission.CaseStage.QUOTE_ACCEPTED)
        sub.refresh_from_db()
        self.assertTrue(sub.can_send_contract_package())

        sub.advance_case_stage(SurveySubmission.CaseStage.PAYMENT_COMPLETED)
        sub.refresh_from_db()
        self.assertTrue(sub.is_ready_for_availability_request())

        sub.advance_case_stage(SurveySubmission.CaseStage.LSA_PENDING)
        sub.refresh_from_db()
        self.assertTrue(sub.is_ready_for_schedule_finalize())
...
class SettlementQuoteAndSubmissionFlowTests(TestCase):
    ...
    def test_finalize_and_send_quote_sets_submission_awaiting_payment(self, mock_email, mock_msg):
        ...
        self.assertEqual(self.submission.case_stage, SurveySubmission.CaseStage.QUOTE_SENT)
        self.assertIsNotNone(self.submission.case_stage_updated_at)
    ...
    def test_process_quote_payment_sets_submission_agent_assignment(self, mock_build, mock_ensure, mock_auto_draft):
        ...
        self.submission.case_stage = SurveySubmission.CaseStage.QUOTE_SENT
        self.submission.save(update_fields=['status', 'case_stage'])
