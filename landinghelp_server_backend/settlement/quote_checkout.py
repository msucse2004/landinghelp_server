...
def process_quote_payment(user, quote_id=None):
    ...
    if quote.submission_id and not quote.submission.is_ready_for_payment():
        return None, '현재 케이스 단계에서는 결제를 진행할 수 없습니다.'
    ...
    if quote.submission_id:
        try:
            from .availability_request import create_post_payment_availability_requests

            create_post_payment_availability_requests(quote.submission, actor=user, notify=True)
        except Exception as e:
            logger.warning(
                'Quote %s payment completed but availability request creation failed: %s',
                quote.id,
                e,
                exc_info=True,
            )
