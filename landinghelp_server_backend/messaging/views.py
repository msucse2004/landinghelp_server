...
from .workspace import can_user_access_workspace_conversation
...
def _pick_staff_sender_for_conversation(conv: Conversation):
    ...
    if not conv:
        return None
    internal_roles = User.internal_staff_role_values()
    staff_participant = (
        ConversationParticipant.objects.filter(conversation=conv).filter(
            models.Q(user__role__in=internal_roles) | models.Q(user__is_staff=True)
        )
        .select_related('user')
        .order_by('id')
        .first()
    ...
    try:
        return User.objects.filter(
            models.Q(role__in=internal_roles) | models.Q(is_staff=True)
...
def _conversation_display_titles(conv, preferred_lang, viewer_user=None):
    ...
    if conv.type == Conversation.Type.NOTICE:
        sub = getattr(conv, 'survey_submission', None)
        if sub:
            if getattr(conv, 'workspace_type', Conversation.WorkspaceType.OTHER) == Conversation.WorkspaceType.HQ_BACKOFFICE:
                base_label = get_display_text('본사업무방', pref) or '본사업무방'
                base_label_en = get_display_text('본사업무방', 'en') or 'HQ Workspace'
            elif getattr(conv, 'workspace_type', Conversation.WorkspaceType.OTHER) == Conversation.WorkspaceType.LOCAL_EXECUTION:
                base_label = get_display_text('현지진행방', pref) or '현지진행방'
                base_label_en = get_display_text('현지진행방', 'en') or 'Local Execution Workspace'
            else:
...
def api_conversation_list(request):
    ...
    out = []
    for conv in qs:
        if not can_user_access_workspace_conversation(user, conv):
            continue
...
def api_conversation_messages(request, conversation_id):
    ...
    if request.method == 'POST':
        ...
        try:
            if not user.can_participate_in_hq_workspace():
...
def api_create_notice(request):
    ...
    """
    if not request.user.can_participate_in_hq_workspace():
...
def api_conversation_detail(request, conversation_id):
    ...
    try:
        ...
        payload = {
            ...
            'created_at': conv.created_at.isoformat(),
            'updated_at': conv.updated_at.isoformat(),
            'pending_reopen_proposal_id': None,
            'pending_reopen_offer_id': None,
        }
        if user.can_participate_in_hq_workspace() and getattr(conv, 'survey_submission_id', None):
        ...
        sub = getattr(conv, 'survey_submission', None)
        if (
            not user.can_participate_in_hq_workspace()
            and sub
        ...
        if not user.can_participate_in_hq_workspace():
            try:
                ...
                for item in (payload.get('pending_actions') or []):
                    if str(item.get('button_action_key') or '').lower() == 'reopen_survey':
                        pending_reopen = item
                        break
                if not pending_reopen:
                    for item in (payload.get('action_offers') or []):
                        if (
                            str(item.get('button_action_key') or '').lower() == 'reopen_survey'
                            and str(item.get('status') or '').upper() == 'PENDING'
                        ):
                            pending_reopen = item
                            break
                if pending_reopen:
                    payload['pending_reopen_proposal_id'] = pending_reopen.get('proposal_id') or None
                    if not payload['pending_reopen_proposal_id']:
                        payload['pending_reopen_offer_id'] = pending_reopen.get('id') or None
            except Exception as e:
                import logging
                logging.getLogger(__name__).warning(
            ...
            if not payload.get('pending_reopen_proposal_id'):
                try:
                    from messaging.models import CustomerActionProposal
                    latest_pending = (
                        CustomerActionProposal.objects
                    ...
                    if latest_pending:
                        payload['pending_reopen_proposal_id'] = latest_pending.id
                except Exception:
                    pass
