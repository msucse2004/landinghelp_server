...
_SEMANTIC_INTENT_EXAMPLES: Dict[Intent, Tuple[str, ...]] = {
    Intent.SURVEY_REOPEN_REQUEST: (
        "설문 내용을 다시 수정하고 싶어요",
        "입력한 정보를 변경하고 싶어요",
        "설문 제출했는데 공항 픽업을 추가하고 싶어요",
        "생각해보니 공항 픽업도 필요해요",
        "i also need an airport pickup",
        "비행기 도착 시간이 변경됐어요",
        "비행 스케줄이 바뀌었어요",
        "비행 스캐쥴이 바뀌었어요",
        "비행 스캐줄이 바꼈어요",
...
def _apply_safety_gate(
    ...
    if (
        base_policy.detected_intent == Intent.SURVEY_REOPEN_REQUEST
        and base_policy.detected_intent != candidate.detected_intent
        and base_policy.confidence >= 0.7
        and is_high_risk_intent(candidate.detected_intent)
    ):
        return None
...
_ADDITIONAL_REQUEST_SIGNAL_GROUPS: Dict[str, Tuple[str, ...]] = {
    "need": (
        "필요",
        "원해",
        "원합니다",
    ...
    "additive": ("추가", "더", "또", "또한", "also", "additional"),
}
...
def _match_post_submission_additional_service_request(
    normalized_text: str,
    context: Optional[Dict[str, Any]] = None,
) -> Optional[Tuple[List[str], List[str], float]]:
    if not (context and context.get("has_survey_submission")):
        return None
    ...
    additional_matches = _collect_signal_group_matches(normalized_text, _ADDITIONAL_REQUEST_SIGNAL_GROUPS)
    if not additional_matches:
        return None

    if not any(group in field_matches for group in ("airport", "service")):
        return None

    confidence = 0.73
    if len(field_matches) >= 2:
        confidence += 0.02
    if "additive" in additional_matches:
        confidence += 0.02
    return field_matches, additional_matches, min(confidence, 0.79)
...
def _heuristic_policy(text: str, context: Optional[Dict[str, Any]] = None) -> PolicyResult:
    ...
    additional_service_match = _match_post_submission_additional_service_request(
        normalized_text,
        context=context,
    )
    if additional_service_match:
        field_matches, additional_matches, confidence = additional_service_match
        ...
        return PolicyResult(
            ...
            human_review_target=entry.human_review_target,
            customer_facing_summary="추가로 필요한 서비스가 있어 설문 수정을 원하시는 것 같아요. 설문을 다시 열어드릴까요?",
            internal_reasoning_summary=(
                "matched post-submission additional service request field groups="
                + ",".join(field_matches)
                + " additional signals="
