...
def get_agent_availability_for_submission(submission, schedule_plan=None):
    ...
    def _append_window(agent_id, start_dt, end_dt):
        if not agent_id or not start_dt or not end_dt or end_dt <= start_dt:
            return
        if agent_id not in by_agent:
            by_agent[agent_id] = []
        by_agent[agent_id].append((start_dt, end_dt))

    def _merge_internal_availability_windows():
        if not submission:
            return
        from django.utils import timezone
        ...
        horizon_days = 28
        request_qs = AvailabilityRequest.objects.filter(
            submission=submission,
            participant_role=AvailabilityRequest.ParticipantRole.AGENT,
            status=AvailabilityRequest.Status.RESPONDED,
        ).prefetch_related('responses__slots')
        if schedule_plan and schedule_plan.id:
            request_qs = request_qs.filter(
                models.Q(schedule_plan=schedule_plan) | models.Q(schedule_plan__isnull=True)
            )

        for availability_request in request_qs:
            latest_response = availability_request.responses.order_by('-revision', '-submitted_at').first()
            if not latest_response:
                continue
            for slot in latest_response.slots.all():
                slot_timezone_name = (
                    (slot.timezone_name or '').strip()
                    or (latest_response.timezone_name or '').strip()
                    or (availability_request.timezone_name or '').strip()
                    or 'UTC'
                )
                try:
                    slot_tz = ZoneInfo(slot_timezone_name)
                except Exception:
                    slot_tz = ZoneInfo('UTC')
                for offset in range(horizon_days):
                    target_day = today + timedelta(days=offset)
                    if target_day.weekday() != int(slot.weekday):
                        continue
                    start_local = datetime.combine(target_day, slot.start_time, tzinfo=slot_tz)
                    end_local = datetime.combine(target_day, slot.end_time, tzinfo=slot_tz)
                    if end_local <= start_local:
                        continue
                    start_dt = start_local.astimezone(timezone.get_current_timezone())
                    end_dt = end_local.astimezone(timezone.get_current_timezone())
                    _append_window(availability_request.target_user_id, start_dt, end_dt)
    ...
    if submission:
        ...
        for w in qs:
            _append_window(w.agent_id, w.starts_at, w.ends_at)
    if schedule_plan and schedule_plan.id:
        qs = AgentAvailabilityWindow.objects.filter(
    ...
    _merge_internal_availability_windows()

    for aid, windows in list(by_agent.items()):
        dedup = sorted({(s, e) for s, e in windows}, key=lambda row: row[0])
        by_agent[aid] = dedup
    return by_agent
